// Divergence debug unit tests | Spencer Tipping <spencer@spencertipping.com>
...
// Unit test utilities.
  ...
  var print        = this.print ? print : require('sys').puts;
  ...
  d.rebase (function () {
    var f = (n >$> (n > 0 ? n * f (n - 1) : n)).traced ();
    f (3);

    var w          = new d.debug.watcher();
    var factorial  = eval (w.annotate_local (n >$> literal (n > 1 ? n * factorial (n - 1) : n)));
    var factorial2 = eval (w.annotate_local (n >$> (n > 1 ? n * factorial2 (n - 1) : n))).traced();

    print (factorial (2));
    w.log() * d.trace;

    d.trace ('Partial log:');
    w.log().grep (e >$> (e.node.op == '>')) * d.trace;

    d.trace ('Partial log:');
    w.log().grep (e >$> (e.value === 1)) * d.trace;

    w.use_tracing();
    d.trace ('With tracing:');
    factorial (3);

    d.trace ('Factorial2:');
    d.trace ('Annotation for factorial2 is ' + factorial2.original.toString());
    w.predicate = e >$> (! e.value || e.value.constructor !== Function);
    factorial2 (3);
  }) ();